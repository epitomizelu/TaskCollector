...
const TodayTaskScreen: React.FC = () => {
  ...
  useEffect(() => {
    // ✅ 页面加载时，先触发模块激活（确保同步预设任务和今日任务），然后加载任务
    const initializeAndLoad = async () => {
      await triggerModuleActivation();
      await loadTasks();
    };
    initializeAndLoad();
  }, [selectedDate]);
  ...
  /**
   * 触发任务清单模块激活（确保同步预设任务和今日任务）
   */
  const triggerModuleActivation = async () => {
    try {
      // 检查是否已登录
      if (!API_CONFIG.API_KEY || !authService.isLoggedIn()) {
        console.log('未配置 API Key 或用户未登录，跳过模块激活');
        return;
      }

      // 手动触发模块激活逻辑（同步预设任务和今日任务）
      console.log('手动触发任务清单模块激活...');
      
      // 先同步预设任务（强制同步）
      try {
        await taskListService.syncPresetTasksBidirectional(true);
  ...
  const loadTasks = async () => {
    ...
    try {
      ...
      // ✅ 如果本地没有任务，先从云端同步（不强制，避免重复创建）
      if (tasks.length === 0 && API_CONFIG.API_KEY && authService.isLoggedIn()) {
        console.log(`本地没有 ${dateStr} 的任务，开始从云端同步...`);
        try {
          await taskListService.syncDailyTasksFromCloud(dateStr, false); // ✅ 不强制同步
      ...
      } else if (tasks.length === 0 && (!API_CONFIG.API_KEY || !authService.isLoggedIn())) {
      ...
      // 如果是今天，且本地和云端都没有任务，则从预设任务生成（只有在配置了 API_KEY 时才同步）
      if (isToday && tasks.length === 0) {
        console.log('本地和云端都没有今日任务，从预设任务生成今日任务');
        
        // 先同步预设任务（确保预设任务是最新的）- 只有在配置了 API_KEY 且用户已登录时才同步
        if (API_CONFIG.API_KEY && authService.isLoggedIn()) {
          try {
          ...
          console.log('未配置 API Key，跳过预设任务同步，仅使用本地预设任务');
        }
        
        // 从预设任务初始化今日任务（即使没有 API_KEY，也可以使用本地预设任务）
        try {
          await taskListService.initializeTodayTasks();
          tasks = await taskListService.getDailyTasks(dateStr);
        ...
        if (isNewDay) {
          ...
          // 先同步预设任务（只有在配置了 API_KEY 且用户已登录时才同步）
          if (API_CONFIG.API_KEY && authService.isLoggedIn()) {
            try {
              await taskListService.syncPresetTasksBidirectional();
              console.log('预设任务同步完成');
